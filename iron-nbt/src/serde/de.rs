...
use crate::{io::{NbtIoError, Options}, raw::{self, string_from_bytes}};
...
impl<'a, R: Read> Deserializer<'a, R, Unbuffered> {
    ...
    pub fn new(reader: &'a mut R, opts: Options) -> Result<(Self, String), NbtIoError> {
...
impl<'a, R, B, const TAG_ID: u8> DeserializeEnum<'a, R, B, TAG_ID> {
    #[inline]
    fn new(reader: &'a mut R, opts: Options, variant: Cow<'a, str>) -> Self {
        DeserializeEnum {
            reader,
            opts,
            variant,
...
{
    #[inline]
    fn new(reader: &'a mut R, opts: Options) -> Self {
        DeserializeVariant {
            reader,
            opts,
            _buffered: PhantomData,
...
{
    #[inline]
    fn new(reader: &'a mut R, opts: Options) -> DeserializeTag<'a, R, B, TAG_ID> {
        DeserializeTag {
            reader,
            opts,
            _buffered: PhantomData,
    ...
    B: BufferSpecialization<'buffer>,
{
    ...
    where V: serde::de::Visitor<'de> {
        match TAG_ID {
            0x1 => visitor.visit_i8 (raw::read_i8 (self.reader, self.opts)?),
            0x2 => visitor.visit_i16(raw::read_i16(self.reader, self.opts)?),
            0x3 => visitor.visit_i32(raw::read_i32(self.reader, self.opts)?),
            0x4 => visitor.visit_i64(raw::read_i64(self.reader, self.opts)?),
    ...
    where V: Visitor<'de> {
        if TAG_ID == 0x8 {
            if B::BUFFERED {
                ...
                let string = string_from_bytes(bytes, self.opts)?;
...
unsafe impl<'buffer> BufferSpecialization<'buffer> for BufferedCursor<'buffer> {
    ...
    where 'buffer: 'de {
        let cursor: &mut Cursor<&'buffer [u8]> = unsafe {
