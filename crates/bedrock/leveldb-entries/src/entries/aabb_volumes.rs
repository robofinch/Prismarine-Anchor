...
impl AabbVolumesV1 {
    ...
    pub fn extend_serialized(
        ...
        // Structure types
        bytes.extend(structure_types_len.to_le_bytes());
        for (structure_id, identifier) in &self.structure_types {
            ...
            bytes.extend(structure_id.to_le_bytes());
        ...
        // Bounding boxes
        bytes.extend(boxes_len.to_le_bytes());
        for (box_id, volume) in &self.bounding_boxes {
            bytes.extend(box_id.to_le_bytes());
            volume.extend_serialized(bytes);
        }

        // Dynamic spawn areas
        bytes.extend(dynamic_len.to_le_bytes());
        for (box_id, dynamic_area) in &self.dynamic_spawn_areas {
            let full_bounding_box: u32 = if dynamic_area.full_bounding_box {
                1
            } else {
                0
            };
            ...
            bytes.extend(box_id.to_le_bytes());
            bytes.extend(dynamic_area.structure_id.to_le_bytes());
            bytes.extend(full_bounding_box.to_le_bytes());
        }

        // Static spawn areas
        bytes.extend(static_len.to_le_bytes());
        for (box_id, static_area) in &self.static_spawn_areas {
            let full_bounding_box: u32 = if static_area.full_bounding_box {
                1
            } else {
                0
            };
            ...
            bytes.extend(box_id.to_le_bytes());
            bytes.extend(static_area.structure_id.to_le_bytes());
